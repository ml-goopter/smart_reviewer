...
| R21 | Fortune block | An ornamental fortune above the suggestion list, drawn **once per page load** from a bundled corpus. Suggestions stage only. No luck or reward framing |
...
# R21 — the fortune block

A short aphorism sits above the suggestion list, drawn at random when the page
loads. It is decoration: it grounds nothing, is generated by nothing, and the
product works identically without it.

## Where it appears, and where it does not

The suggestions stage of `/r/:token`, **between the merchant identity and the
review block** — below the name and category, above "How was your experience?"
and the cards. It takes a divider of its own beneath it, so the block costs a
rule as well as its own height.

The merchant identity stays first. It is what tells a customer who has just
...
It is **not** on the editor — that screen is where the customer is writing, and
a fortune over the textarea competes with the words they came to edit. It is
not on the loading spinner, `/unavailable`, or the error screen either, which
follows from where it is mounted rather than from a condition: it lives inside
the suggestions screen, which renders only once the session has resolved.

It **is** on screen while the first batch generates, alongside the skeletons.
That is the longest wait a customer sees, and it is the one moment the block
earns its place rather than merely occupying it.

## The corpus

`apps/web/src/lib/fortunes.ts` — 24 entries, bundled at build time. Not fetched:
a request on the critical path competes with the session fetch, would need a
loading and a failure state for a decoration, and buys nothing, because
`vite build` empties `dist/` and the file could not be edited in place anyway.

The shape is `Record<Locale, string>[]` — one entry holding all three
translations, not three arrays keyed by locale:
...
Translations that travel together cannot drift out of step. Parallel arrays
could, and the failure would be a fortune rendering as `undefined` at the tail
of one language only. The non-empty tuple type is what lets the draw narrow
under `noUncheckedIndexedAccess` without an assertion.

A `.ts` module rather than JSON: `tsconfig.json` sets no `resolveJsonModule`
and `npm run build` runs `tsc --noEmit`, so a JSON import would fail the build
until the config changed. The module also types the array against `Locale`, so
adding a locale to `LOCALES` fails the build here rather than rendering blank.

**Two copy rules, both from where the block sits.** Google forbids incentivised
reviews *and* soliciting positive ones, so neither is a matter of taste:

1. Nothing promises luck, reward, or good fortune in return for anything.
2. Nothing steers the review. No kind words, no gentle speech, no candles
   against the darkness, nothing saying the minute spent will be repaid.

Rule 2 is the one that is easy to miss. An aphorism about saying kind things
reads as a request for a kind review directly above that button, and one about
patience or slowness reads, above a restaurant, as a plea to excuse the wait.

A Chinese couplet can break the rule with a line that is not written down:
良言一句三冬暖 is completed by 惡語傷人六月寒, and 海納百川 by 有容乃大. Both ask
for a generous review in a register the English half does not have. **Check what
an entry completes to**, not only what it says.

Read every new entry against both rules, in every language, before adding it.

## Drawn once per page load

The draw lives in `Reviewer`, above the `key={locale}` `<Stage>`. This is the
whole of the decision, and it is invisible in the code:

```text
page load       →  draw entry i
switch language →  entry i, in the new language      not a new draw
back from editor→  entry i                           not a new draw
reload          →  draw again
```

`<Stage>` is keyed on the locale, so a language change remounts everything
below it, and the suggestions screen also unmounts whenever the editor opens.
A draw placed inside `Suggestions` — which reads more naturally — therefore
re-rolls on both, and the fortune shuffles under a customer who only asked to
read the page in Chinese. `Reviewer` outlives both, which is why the draw is
there and why the whole entry is held rather than an index.

`components/fortune.test.tsx` pins this. Nothing else would notice it break.
...
as a rendering fault. Deliberately unlike the white bordered `.card`: a card
above the list reads as a fourth suggestion and invites a tap that does nothing.
